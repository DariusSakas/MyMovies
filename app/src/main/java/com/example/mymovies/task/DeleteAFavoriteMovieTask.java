...
public class DeleteAFavoriteMovieTask extends AsyncTask<FavoriteMovie, Void, Void> {
    ...
    protected Void doInBackground(FavoriteMovie... movies) {
        if (movies != null && movies.length > 0) {
            MovieDatabase.getInstance(context).movieDAO().deleteFavoriteMovie(movies[0]);
        }
        return null;
    }
}
